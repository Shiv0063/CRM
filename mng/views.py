...
def products(request):
    ...
    if request.method=='POST':
        ...
        if name in pl:
            ...
            data =product.objects.create(name=name,status=0)
            data.save()
...
def signup(request):
    if request.method=='POST': 
        ...
        uf1 = User.objects.create(first_name=fname,last_name=lname,username=uname,password=pword,is_staff=True,is_superuser=True,is_active=True)
        uf1.save()
        print("user create")
